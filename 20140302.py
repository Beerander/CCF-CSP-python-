...
for index in range(N):
    windows.append([list(map(lambda x: int(x), input().split(' '))), index+1])
for _ in range(M):
    clicks.append(list(map(lambda x: int(x), input().split(' '))))
for i in range(M):
    x, y = clicks[i]
    for j in range(N-1, -1, -1):
        x1, y1, x2, y2 = windows[j][0]
        if x1 <= x <= x2 and y1 <= y <= y2:
            print(windows[j][1])
            if j != N - 1:
                window = windows.pop(j)
